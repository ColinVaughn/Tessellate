...
import org.texboobcat.tessellate.mixin.accessor.LevelTicksAccessor;
    ...
    private List<ScheduledTick<T>> ticksIn(BoundingBox area) {
        ...
        for (LevelTicks<T> child : schedulers()) {
            locked(child, () -> {
                ...
                accessor.tessellate$alreadyRunThisTick().stream().filter(inside).forEach(result::add);
                accessor.tessellate$toRunThisTick().stream().filter(inside).forEach(result::add);
                Long2ObjectMap<LevelChunkTicks<T>> containers = accessor.tessellate$allContainers();
