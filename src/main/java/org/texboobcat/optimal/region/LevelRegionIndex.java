...
public final class LevelRegionIndex implements RegionizerListener {
    ...
    public interface RegionalLevelAccess {
        void optimal$runVanillaBlockEntityPass();
        Map<Integer, List<TickingBlockEntity>> optimal$prepareRegionalBlockEntities(
            LevelRegionIndex index);
        void optimal$tickRegionalBlockEntities(@Nullable Region region,
                                               List<TickingBlockEntity> tickers);
        void optimal$finishRegionalBlockEntities();
    }

    // Methods added to ServerChunkCache by ServerChunkCacheMixin.
    public interface RegionalChunkAccess {
        void optimal$prepareRegionalChunks(LevelRegionIndex index);
        RegionalChunkWork optimal$regionalChunkWork();
        @Nullable ChunkAccess optimal$getChunkForWorker(int x, int z, ChunkStatus status);
    }
    ...
    private void tickFullRegions(Consumer<Entity> entityConsumer, boolean tickEntityPhase) {
        ...
        RegionalChunkWork chunkWork = chunkAccess.optimal$regionalChunkWork();
        Map<Integer, List<TickingBlockEntity>> blockEntities = tickEntityPhase
            ? levelAccess.optimal$prepareRegionalBlockEntities(this)
            : Map.of();
        ...
        if (tickEntityPhase) {
            levelAccess.optimal$finishRegionalBlockEntities();
        }
    ...
    private void finishUnownedEntityPhase(List<Entity> orphanSnapshot,
        ...
        this.orphans.clearSnapshot();
        levelAccess.optimal$tickRegionalBlockEntities(null,
    ...
    private void submitClaimedRegion(Claimed item,
        ...
        try {
            RegionWorkers.submit(() -> {
                ...
                LOGGER.error("optimal: region tick failed on a worker thread", failure);
            });
    ...
    private void finishAsyncBlockEntityPhase(java.util.concurrent.atomic.AtomicInteger remaining,
        ...
        if (remaining != null && remaining.decrementAndGet() == 0) {
            DeferredMainThreadWork.deferForRegion(
                ...
                this.levelKey, region.id(), levelAccess::optimal$finishRegionalBlockEntities);
        }
    }
    ...
    private void tickFullRegion(RegionRun run, RegionTickState state,
        ...
        try {
            ...
            if (tickEntityPhase) {
                ...
                levelAccess.optimal$tickRegionalBlockEntities(region, blockEntities);
            }
        } finally {
